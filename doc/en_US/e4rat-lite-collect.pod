...
To stop scanning process press STR-C or run `e4rat-lite-collect -k'. Unless otherwise stated, the generated file list is written to '/var/lib/e4rat-lite/startup.log' or the file specified in the configuration.
...
collect while executing command. e4rat-lite-collect stops running after <command> terminates. Be aware that <command> gets executed with root privileges if no username is specified. See --user.
