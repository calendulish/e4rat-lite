...
/etc/e4rat-lite.conf - Arquivo de configuração para o e4rat-lite
...
Options can either be declared below a section. Derivado do nome sufixo do executável, cada binário tem sua própria seção. Valores além dessa seção se aplica ao binário somente.
...
O e4rat-lite possui uma estrutura de log simples. Cada evento 

e4rat-lite has a simple logging structure. Cada evento possui uma prioridade. As seguintes prioridades existem:
    1   erros
    2   avisos
    4   estatisticas da tarefa
...
Defina o <log_targe> para 'syslog' caso queira enviar as mensagens para o daemon do syslog.

=item B<init_file>

Define um processo de inicialização alternativo, substituindo o /sbin/init
...
Ignora arquivo que estão abertos. [Padrão: true]

=item B<timeout>

Depois da expiração desse valor em segundos, o e4rat-lite-collect vai automaticamente terminar a coleta. 
O Timeout apenas é funcional quando o e4rat-lite-collect é executado como um processo de inicialização (init). [Padrão: 120]
