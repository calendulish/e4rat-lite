...
Mata um processo e4rat-lite-collect em execução

=item -x --execute <comando>

Coleta durante a execução do comando. O e4rat-lite-collect para após o <comando> terminar. Esteja ciente de que o <comando> é executado com privilégios de root se nenhum nome de usuário for especificado. Veja --user.

=item -u --user <nome do usuário>

Especifica um nome de usuário para o comando ser executado. Veja --execute.
...
O <arquivo> contém uma lista de arquivos para serem ignorados pelo e4rat-lite-collect

=back

=head1 EXEMPLOS

=head2 Scanear o Thunderbird a localmente rodar o Imap daemon
...
=head2 Scanear processos do boot

Rode o e4rat-lite-collec o mais antes possível para scanear todo o processo de inicialização. É recomendado rodar o e4rat-lite-collect como um processo de inicialização. Para isso, adiciona a seguinte linha aos parametros do kernel em grub(8) ou lilo(8).
